...
\usepackage{wasysym}    % provides \texttimes
\usepackage{enumitem}
\usepackage{float}
\usepackage{gensymb}
\usepackage{url}
...
\usepackage{subcaption}  % Allows subfigure environments
...
\def \srsTitle{AIDosForge: LLM-Powered Game Character Creation System for Unreal Engine 5 (Proposal)}
\def \srsAuthorOne{Pasu Sangiemsin}
\def \srsAuthorTwo{Kasidet Uthaiwiwatkul}
\def \srsAuthorThree{Riccardo Mario Bonato}
\def \srsAcademicYear{2027}

\def \srsAdvisorName{Dr. Chawanat Nakasan}
\def \srsCoAdvisorName{Project Co-Advisor's Name}
\def \srsHoDName{Head of Department's Name}
\def \brandName{AIDosForge}
%% -----------------------------------
